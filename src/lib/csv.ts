...
export async function generateXlsxReport(client: Client, building: Building, extinguishers: Extinguisher[], hoses: Hydrant[]) {
    return new Promise<void>((resolve) => {
        ...
        const extBody = (extinguishers || []).map(e => {
            ...
            return [alert, e.id, e.observations, e.type, e.weight, formatDate(e.expiryDate), e.hydrostaticTestYear, insp.status, insp.notes];
        });
        ...
        const hoseBody = (hoses || []).map(h => {
            ...
            return [alert, h.id, h.location, h.quantity, 'Tipo ' + h.hoseType, h.diameter + '"', h.hoseLength, h.keyQuantity, h.nozzleQuantity, formatDate(h.hydrostaticTestDate), insp.status, insp.notes];
        });
...
export async function generateClientXlsxReport(client: Client, buildings: Building[]) {
    return new Promise<void>((resolve) => {
        ...
        // --- Hoses Section ---
        const hoseHeader = ['Alerta', 'ID', 'Prédio', 'Local', 'Qtd Mangueiras', 'Tipo', 'Diâmetro', 'Medida (m)', 'Qtd Chaves', 'Qtd Esguichos', 'Próx. Teste Hidr.', 'Status Últ. Insp.', 'Observações Últ. Insp.'];
        ...
        const hoseBody = allHoses.map(h => {
            ...
            return [ alert, h.id, h.buildingName, h.location, h.quantity, h.hoseType, h.diameter, h.hoseLength, h.keyQuantity, h.nozzleQuantity, formatDate(h.hydrostaticTestDate), insp.status, insp.notes ];
        });
        const wsHose = XLSX.utils.aoa_to_sheet([hoseHeader, ...hoseBody]);
        applyAutoFilter(wsHose, hoseHeader.length);
        XLSX.utils.book_append_sheet(wb, wsHose, 'Hidrantes');
